...
//		alert(dragObj.style.left);
	...
	function show_img(url_,floor_current){
...
//			img_obj.onload_();
	...
	show_img('show_map.php?name_img='+arr_floor["F1"]["img_src"]+"&id_build="+$("#id_build").val()+"&sid="+Math.random(),"F1");
	
	$("input[type='button'][guiji]").each(function(i){
		$(this).live("click",function(){
			show_img('show_map.php?name_img='+arr_floor[this.value]["img_src"]+"&id_build="+$("#id_build").val()+"&sid="+Math.random(),this.value);
		});											   
	});
...
//					alert(e.message); 
	...
	function show_guiji(){
		//alert("maps_building/show_guiji_line.php?bigclassid="+$("#id_build").val()+"&Ddate2="+$("#show_img").attr("Ddate2")+"&danw="+$("#show_img").attr("danw")+"&scale="+scale+"&Dyear="+$("#Dyear").val()+"&Dmonth="+$("#Dmonth").val()+"&Dday="+$("#Dday").val()+"&Dhour="+$("#Dhour").val()+"&user="+encodeURIComponent($("#user").val())+"");
		$.post("maps_building/show_guiji_line_3.php", { 
				bigclassid: $("#id_build").val(), 
				floor_list: $("#floor").val(), 
